...
- **[HIP Hosting](https://hip.hosting/?code=e7ba57714b589e715371)** ⭐ — лучшее соотношение цены и мощности. Принимают крипту. Серверы бывают не всегда в наличии, но выцепить реально — просто заходи периодически и смотри
...
Каждый сигнал теперь показывает разницу цен между двумя биржами и сколько дней нужно держать позицию, чтобы выйти в плюс с учётом входа и выхода:

```
💱 Спред цен: ~0.05% (туда-обратно: ~0.10%)
⏳ Окупится: ~1.2д
```

Это помогает оценить реальную привлекательность пары — не только по APR, но и с учётом стоимости открытия.

### Чёрный список токенов

В каждом сигнале есть кнопка **🚫 В чёрный список**. Нажал — и этот токен больше не появляется в сигналах.

Управлять списком можно в **⚙️ Настройки → Чёрный список**: видишь все заблокированные токены, удаляешь одним тапом.

### Пресеты настроек

Можно сохранить любой набор настроек под именем и быстро переключаться между ними.

1. Настрой всё как нужно
2. В **⚙️ Настройки → Пресеты** нажми **💾 Сохранить текущие**
3. Введи название, например: `Консервативный` или `Только HL+BP`
4. Чтобы применить — нажми **▶** рядом с нужным пресетом

Пресет сохраняет: включённые биржи, размер позиций, настройки защиты.
...
- Настроить параметры автоматической защиты (см. ниже)
- Управлять чёрным списком токенов
- Создавать и применять пресеты настроек

### Автоматическая защита

Бот закрывает пару автоматически если:
- Нетто APR опустился ниже **−50%**
- APR стал отрицательным и не восстановился за настроенное время
- Цена ушла против любой ноги более чем на настроенный порог от входа
- До ликвидации любой ноги осталось менее **15%** (предупреждение приходит на 20%)

Все пороги защиты настраиваются прямо в боте — **⚙️ Настройки → Защита позиций**:
- **Вкл/Выкл** — полностью включить или выключить автозащиту
- **% падения цены** — закрыть если цена ушла против ноги на это значение (пресеты: 15 / 25 / 35 / 50 / 100%, или вручную)
- **Часов в минус-фандинге** — закрыть если Net APR остаётся отрицательным это время (пресеты: 1 / 2 / 4 / 6 / 12 / 24ч, или вручную)

Настройки сохраняются в базе данных и не сбрасываются при перезапуске.

### Стрик фандинга в сигналах

В каждом сигнале и при ручном скане показывается, как давно Net APR пары держится положительным:

```
Net APR: ~38.5% ⏱ 47ч
```

Это помогает выбирать пары с проверенной историей. Короткие просадки в минус (до 4 часов) игнорируются и не сбрасывают счётчик. Счётчик сбрасывается при перезапуске бота.

---

## Обновление до последней версии

Подключись к серверу по SSH и выполни три команды:
...
> 💡 База данных и файл `.env` при обновлении не трогаются — все позиции, настройки и ключи сохраняются.
